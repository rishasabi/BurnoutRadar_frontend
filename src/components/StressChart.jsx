...
export default function StressChart({ data = [], labels = DAYS }) {

  if (!data || data.length === 0) {
    return (
      <p style={{ color: '#9a8aaa', fontSize: 12 }}>
  ...
  // Auto-scale max value (minimum 100 to keep % scale)
  const maxV = Math.max(...data, 100);

  // Prevent division by zero when only 1 data point
  const safeDivisor = Math.max(data.length - 1, 1);

  const pts = data
    .map((v, i) => {
      const x = (i / safeDivisor) * w;
      const y = h - (v / maxV) * h;
      return `${x},${y}`;
    })
    .join(' ');
